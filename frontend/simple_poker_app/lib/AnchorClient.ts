...
export async function initLobby(
  ...
  const lobbyAccountInfo = await program.account.gameLobby.getAccountInfo(
    lobbyPDA
  )
  ...
  try {
    const tx_signature = await program.methods
      ...
      .rpc()
    console.log('Game lobby initialized successfully!', `Tx: ${tx_signature}`)
    return true
  } catch (error) {
...
export async function isLobbyInitialized(
  program: anchor.Program<SimplePoker>
): Promise<boolean> {
  if (!program) {
...
export async function CreateGame(
  ...
  if (gameAccountInfo) {
    console.log(
      'A Game already initialized. Please Join the game or wait for it to complete'
    )
...
export async function ClaimWinnings(
  ...
  if (
    gameAccountData.winner?.toBase58() == provider.wallet.publicKey.toBase58()
  ) {
    try {
      const tx_signature = await program.methods
        .claimPrize()
...
const mapOnChainDataToGame = (account: any): Game => {
  ...
  if (account.prizePool.toNumber() === 0) {
    prizePool =
      (account.stakeAmount.toNumber() * account.maxPlayers) /
  ...
  return {
    ...
    stakingAmount:
      account.stakeAmount.toNumber() / anchor.web3.LAMPORTS_PER_SOL,
