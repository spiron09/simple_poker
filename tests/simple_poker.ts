...
describe('simple_poker', () => {
  ...
  it('Game Lobby Init', async () => {
    ...
    assert.strictEqual(
      Number(lobbyAccount.currentGameId),
      0,
      'Last game ID should be 0'
    )
  })
  ...
  it('Determines the winner of the game', async () => {
    ...
    assert.ok(gameAccount.winner !== null, 'Winner should be determined')
    ...
    assert.ok(isValidWinner, 'Winner should be one of the game players')
  ...
  it('Claims the prize', async () => {
    ...
    console.log(
      'Vault amount:',
      Number(vaultBalance) / anchor.web3.LAMPORTS_PER_SOL
    )
    ...
    assert.ok(gameAccount_data.isClaimed, 'Game should be marked as claimed')
    try {
      const vaultBalance = await provider.connection.getBalance(vaultPDA)
